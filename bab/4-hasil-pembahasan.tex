...
Tahap akhir \textit{deduplication} per-\textit{track} menghasilkan 2 \textit{event} pelanggaran unik dari keseluruhan 3.001 \textit{frame} video. Dengan demikian, hanya 0,02\% kandidat awal yang dipertahankan sebagai pelanggaran akhir setelah seluruh lapisan filter diterapkan.
...
Tabel~\ref{tab:latency-decomposition} menunjukkan bahwa sebagian besar waktu pemrosesan sistem berada pada tahap inferensi model deteksi. Nilai P95 sistem tercatat sebesar 57,52 milidetik dengan deviasi standar total 3,23 milidetik. Dibandingkan modul inferensi, latensi ByteTrack dan modul \textit{filtering} relatif kecil sehingga kontribusinya terhadap total waktu pemrosesan tidak signifikan.
...
Tabel~\ref{tab:kafka-throughput} memperlihatkan bahwa \textit{throughput producer} mencapai 11.485 pesan per detik dengan \textit{bandwidth} rata-rata 3,76 MB per detik. Ukuran rata-rata setiap pesan tercatat sebesar 335,6 \textit{byte}, sedangkan median latensi transmisi berada pada 2,65 milidetik. Seluruh pesan diterima tanpa kehilangan data selama proses pengiriman berlangsung.
...
Dua kueri pada \textit{tier} 3 membutuhkan waktu lebih dari 6 detik. BM-20 yang menghitung statistik ringkas tahunan selesai dalam 6.559,35 ms, sedangkan BM-12 dengan \textit{window function} geospasial membutuhkan 6.949,09 ms. Kedua kueri tersebut memiliki kompleksitas agregasi dan pemindaian data yang lebih besar dibanding kueri lain.
