...
Pengembangan dan pengujian sistem dilakukan pada satu unit laptop dengan spesifikasi perangkat keras yang ditunjukkan pada Tabel~\ref{tab:spek-hardware}. Sistem dibangun sebagai purwarupa (\textit{prototype}) yang berjalan penuh pada satu perangkat tersebut, bukan sebagai penyebaran lintas banyak komputer.
...
Layanan pendukung arsitektur Kappa, yaitu Apache Kafka beserta Zookeeper, Apache Spark (mode \textit{master} dan \textit{worker}), basis data PostgreSQL, dan Grafana, dijalankan sebagai kontainer Docker yang diorkestrasi melalui \textit{Docker Compose} dan saling terhubung pada satu jaringan \textit{bridge} internal. Adapun \textit{pipeline} deteksi (YOLOv8 dan ByteTrack) dijalankan secara \textit{native} menggunakan \textit{runtime} Python pada laptop yang sama. Pemisahan antarsubsistem, yaitu subsistem deteksi, mesin pemrosesan \textit{stream} (Spark), basis data, dan visualisasi Grafana, disimulasikan melalui kontainer dan jendela terminal yang berbeda pada satu perangkat. Dengan demikian, komunikasi yang pada sistem berskala nyata berlangsung antarperangkat di sini berlangsung antarlayanan di dalam satu perangkat. Konfigurasi \textit{master--worker} pada Spark juga merupakan simulasi klaster pemrosesan pada satu mesin, bukan klaster fisik multinode.
...
Secara keseluruhan, keenam komponen tersebut membentuk alur kerja dari video mentah menjadi informasi analitik yang siap digunakan. Kafka berperan sebagai penyangga agar aliran data tetap stabil, sedangkan Spark Structured Streaming memastikan data yang masuk ke penyimpanan sudah melalui proses validasi dan pengolahan awal. Hasil olahan disimpan dalam \textit{data mart} agar dapat ditampilkan secara ringkas melalui Grafana dan dimanfaatkan untuk pemantauan pelanggaran helm secara operasional. Pemisahan keenam lapisan ini bersifat logis: pada purwarupa, lapisan \textit{buffering}, \textit{stream processing}, \textit{persistence}, dan \textit{presentation} dikemas sebagai kontainer Docker, sedangkan lapisan \textit{ingestion} dan inferensi berjalan sebagai proses \textit{native}, semuanya pada satu laptop. Tiap lapisan dijalankan pada kontainer atau terminal terpisah untuk menyimulasikan komunikasi antarsubsistem, sehingga rancangan yang sama dapat dipindahkan ke beberapa perangkat fisik tanpa mengubah alur datanya.
...
\begin{enumerate}[a.]
  ...
  \item {Asosiasi Spasial \texttt{DRIVER\_NO\_HELMET}--\texttt{MOTORCYCLE} (IoA Filter)}: setelah pelacakan, dilakukan validasi spasial antara \textit{bounding box} kelas \texttt{DRIVER\_NO\_HELMET} dengan \textit{bounding box} kelas \texttt{MOTORCYCLE} menggunakan metrik \textit{Intersection over Area} (IoA). IoA mengukur proporsi area \textit{bounding box} \texttt{DRIVER\_NO\_HELMET} yang beririsan dengan area \texttt{MOTORCYCLE}:
    ...
    \noindent dengan:
    \begin{description}
      \item[$\text{bbox}_{\texttt{DNH}}$:] \textit{bounding box} kelas \texttt{DRIVER\_NO\_HELMET} sebagai kotak acuan;
      \item[$\text{bbox}_{\texttt{MC}}$:] \textit{bounding box} kelas \texttt{MOTORCYCLE} sebagai kotak pembanding;
      \item[$|\text{bbox}_{\texttt{DNH}} \cap \text{bbox}_{\texttt{MC}}|$:] luas area irisan antara kedua kotak, dalam satuan piksel;
      \item[$|\text{bbox}_{\texttt{DNH}}|$:] luas area kotak \texttt{DRIVER\_NO\_HELMET}, dalam satuan piksel.
    \end{description}
    Metrik IoA dipilih ketimbang IoU karena \textit{bounding box} \texttt{DRIVER\_NO\_HELMET} umumnya jauh lebih kecil daripada \texttt{MOTORCYCLE}, sehingga IoU menghasilkan skor rendah meskipun pengendara berada di area sepeda motor. Deteksi \texttt{DRIVER\_NO\_HELMET} dianggap valid mengendarai motor jika IoA $\geq$ 0{,}3. Deteksi \texttt{DRIVER\_NO\_HELMET} tanpa asosiasi terhadap \texttt{MOTORCYCLE} (misalnya pejalan kaki) disaring pada tahap ini.
...
Tahap ini membangun pipeline streaming untuk mengubah deteksi mentah menjadi \textit{event} terverifikasi. Layanan streaming dijalankan sebagai kontainer Docker yang terpisah pada satu mesin, sehingga karakter pemrosesan antarlayanan tetap terjaga tanpa memerlukan banyak perangkat fisik.
\begin{enumerate}[a.]
